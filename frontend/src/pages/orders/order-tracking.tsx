...
const OrderTrackingPage = () => {
  ...
  if (isLoading)
    return (
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 py-2">
        <Button
          variant="link"
          className="w-fit px-0 text-muted-foreground animate-pulse"
          asChild
        >
          <Link to="/orders">
  ...
  const currentStepIndex =
    STEP_CONFIG.findIndex((s) => s.status === normalizedStatus) === -1
            ...
            <div className="relative flex items-center justify-between w-full">
              ...
              {STEP_CONFIG.map((step, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`grid size-9 place-items-center rounded-full border-2 transition-all duration-300 ${isCompleted ? 'border-primary bg-primary text-primary-foreground shadow-md' : isCurrent ? 'border-primary bg-primary text-primary-foreground ring-4 ring-primary/20 scale-110 font-bold shadow-md' : 'border-muted-foreground/30 text-muted-foreground bg-background'}`}
                    >
                      <Icon className="size-4" />
                    </div>
                    <span
                      className={`mt-2 text-[10px] md:text-xs font-semibold ${isCurrent ? 'text-primary font-bold' : isActive ? 'text-foreground' : 'text-muted-foreground'}`}
                    >
                      {step.label}
                    </span>
                  </div>
