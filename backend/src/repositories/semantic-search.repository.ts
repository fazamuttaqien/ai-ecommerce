...
export type SemanticSearchFilters = { categoryId?: string; brand?: string; minPrice?: number; maxPrice?: number };

export type SemanticSearchRepositoryResult = {
  id: string; name: string; brand: string; slug: string; description: string | null; images: string[];
  originalPrice: number; salePrice: number; discountPercent: number; unit: string; stockCount: number;
  ratingAverage: number; reviewCount: number; category: { id: string; name: string } | null; similarity: number;
};

export type SemanticSearchRepositoryResponse = { items: SemanticSearchRepositoryResult[]; total: number };

export class SemanticSearchRepository {
  async search(queryEmbedding: number[], filters: SemanticSearchFilters, page: number, pageSize: number): Promise<SemanticSearchRepositoryResponse> {
    const offset = (page - 1) * pageSize;
    const distance = cosineDistance(productEmbeddings.embedding, queryEmbedding);
    const similarity = sql<number>`1 - (${distance})`;
    const conditions = [eq(productEmbeddings.model, embeddingConfig.model), eq(products.isActive, true), gte(similarity, semanticSearchConfig.similarityThreshold)];
