...
export class RecommendationRepository {
  async getUserRecommendationData(
    ...
    const purchaseSignals = aggregate(
      purchases.map((row) => ({ productId: row.productId, weight: Math.max(1, row.quantity) })),
    );
    const interactionSignals = aggregate(
      interactions.map((row) => ({
        productId: row.productId,
        weight: row.type === 'homepage_click' ? 2 : 1,
      })),
    );
    const cartSignals = aggregate(
      cart.map((row) => ({ productId: row.productId, weight: Math.max(1, row.quantity) })),
    );
    ...
    const [preferenceProducts, candidates] = await Promise.all([
      ...
      db
        ...
        .where(and(eq(products.isActive, true), ne(products.stockCount, 0)))
        .orderBy(desc(products.reviewCount), desc(products.ratingAverage), desc(products._id))
