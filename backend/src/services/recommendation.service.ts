  ...
  async getRecommendations(userId: string, limit = recommendationConfig.defaultLimit): Promise<RecommendationResponse> {
    ...
    for (const product of data.preferenceProducts) {
      ...
      if (product.category?.id) {
        categoryAffinity.set(product.category.id, (categoryAffinity.get(product.category.id) ?? 0) + weight);
      }
    ...
    const priceValues = data.preferenceProducts
      .filter((product) => seedWeights.has(product.id))
      .map((product) => product.salePrice);
    const preferredPrice = priceValues.length
    ...
    const scored = data.candidates.map((product) => {
      ...
      const categoryScore = product.category?.id
        ? normalize(categoryAffinity.get(product.category.id) ?? 0, maxCategoryAffinity)
        : 0;
      const brandScore = normalize(brandAffinity.get(product.brand) ?? 0, maxBrandAffinity);
      const semanticScore = preferenceVector && product.embedding
        ? clamp((cosineSimilarity(preferenceVector, product.embedding) + 1) / 2)
        : 0;
      ...
      const priceScore = preferredPrice > 0
        ? clamp(1 - Math.abs(product.salePrice - preferredPrice) / Math.max(preferredPrice, product.salePrice, 1))
        : 0;
    ...
    while (selected.length < limit && remaining.length) {
      ...
      for (let index = 0; index < remaining.length; index += 1) {
        ...
        const categoryPenalty = categoryId
          ? (categoryCounts.get(categoryId) ?? 0) * recommendationConfig.diversity.categoryPenalty
          : 0;
