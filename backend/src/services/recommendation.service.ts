...
export type RecommendationResultItem = Omit<
...
export class RecommendationService {
  ...
  async getRecommendations(
    ...
    const data = await this.repository.getUserRecommendationData(
      userId,
      recommendationConfig.candidateLimit,
      recommendationConfig.preferenceSeedLimit,
    );

    const positiveSignals = new Map<string, number>();
    const addSignals = (
      signals: Array<{ productId: string; weight: number }>,
      multiplier: number,
    ) => {
      for (const signal of signals) {
        positiveSignals.set(
          signal.productId,
          (positiveSignals.get(signal.productId) ?? 0) +
    ...
    const seedWeights = new Map(
      [...positiveSignals.entries()]
        .sort((left, right) => right[1] - left[1])
        .slice(0, recommendationConfig.preferenceSeedLimit),
    );
    ...
    for (const product of data.preferenceProducts) {
      ...
      if (product.category?.id)
        categoryAffinity.set(
          product.category.id,
          (categoryAffinity.get(product.category.id) ?? 0) + weight,
        );
      brandAffinity.set(
        product.brand,
        (brandAffinity.get(product.brand) ?? 0) + weight,
      );
    }
    ...
    const weightedPrice = data.preferenceProducts
      ...
      .reduce(
        (result, product) => {
          const weight = seedWeights.get(product.id) ?? 0;
          return {
            total: result.total + product.salePrice * weight,
            weight: result.weight + weight,
          };
        },
        { total: 0, weight: 0 },
      );
    const preferredPrice = weightedPrice.weight
      ? weightedPrice.total / weightedPrice.weight
      : 0;
    const maxBehavior = Math.max(1, ...positiveSignals.values());

    const scored: ScoredRecommendation[] = data.candidates.map((product) => {
      const behavioralScore = normalize(
        positiveSignals.get(product.id) ?? 0,
        maxBehavior,
      );
      const categoryScore = product.category?.id
        ? normalize(
            categoryAffinity.get(product.category.id) ?? 0,
            maxCategoryAffinity,
          )
        : 0;
      const brandScore = normalize(
        brandAffinity.get(product.brand) ?? 0,
        maxBrandAffinity,
      );
      const semanticScore =
        preferenceVector && product.embedding
          ? clamp(
              (cosineSimilarity(preferenceVector, product.embedding) + 1) / 2,
            )
          : 0;
      const popularityScore = clamp(
        Math.log1p(product.reviewCount) / Math.log1p(100),
      );
      ...
      const priceScore =
        preferredPrice > 0
          ? clamp(
              1 -
                Math.abs(product.salePrice - preferredPrice) /
                  Math.max(preferredPrice, product.salePrice, 1),
            )
          : 0;
      const score =
    ...
    const nonPurchased = scored.filter(
      (item) => !purchasedIds.has(item.product.id),
    );
    ...
    return {
      items: selected.map(({ product, score, ...signals }) => {
        ...
        return {
          ...
          recommendationReason: this.getReason(
            product,
            signals,
            data,
            categoryAffinity,
  ...
  private getReason(
    ...
    if (data.purchases.some((item) => item.productId === product.id))
      return 'recent-purchases';
    if (data.cart.some((item) => item.productId === product.id))
      return 'cart-interest';
    if (
      signals.categoryScore >= 0.65 &&
      product.category?.id &&
      categoryAffinity.has(product.category.id)
    )
      return 'liked-category';
    if (signals.brandScore >= 0.65 && brandAffinity.has(product.brand))
      return 'liked-brand';
    if (preferenceVector && product.embedding && signals.semanticScore >= 0.72)
      return 'similar-to-history';
    if (
      signals.discountScore >= 0.25 &&
      (signals.categoryScore >= 0.35 ||
        signals.brandScore >= 0.35 ||
        signals.semanticScore >= 0.55 ||
        signals.priceScore >= 0.65)
    )
  ...
  private buildPreferenceVector(
    ...
    const vectors = data.preferenceProducts.filter(
      (product) => product.embedding && seedWeights.has(product.id),
    );
    ...
    for (const product of vectors) {
      ...
      for (let index = 0; index < dimensions; index += 1)
        result[index] += embedding[index] * weight;
    }
  ...
  private applyDiversity(
    ...
    while (selected.length < limit && remaining.length) {
      ...
      for (let index = 0; index < remaining.length; index += 1) {
        ...
        const categoryPenalty = categoryId
          ? (categoryCounts.get(categoryId) ?? 0) *
            recommendationConfig.diversity.categoryPenalty
          : 0;
        const brandPenalty =
          (brandCounts.get(item.product.brand) ?? 0) *
          recommendationConfig.diversity.brandPenalty;
        const similarityPenalty =
          selected.reduce((maxSimilarity, chosen) => {
            if (!chosen.product.embedding || !item.product.embedding)
            ...
            return Math.max(
              ...
              Math.max(
                0,
                cosineSimilarity(
                  chosen.product.embedding,
                  item.product.embedding,
                ) - 0.75,
              ),
            );
          }, 0) * recommendationConfig.diversity.similarityPenalty;
        const diverseScore =
          item.score - categoryPenalty - brandPenalty - similarityPenalty;
        if (
          diverseScore > bestScore ||
          (diverseScore === bestScore &&
            item.product.id.localeCompare(remaining[bestIndex].product.id) < 0)
        ) {
      ...
      brandCounts.set(
        ...
        (brandCounts.get(chosen.product.brand) ?? 0) + 1,
      );
    }
    return selected;
  }
