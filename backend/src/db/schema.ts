...
export const products = pgTable('products', {
  ...
  categoryId: uuid('category_id').notNull().references(() => categories._id),
...
}, (table) => [unique('product_embeddings_product_model_unique').on(table.productId, table.model), index('product_embeddings_product_id_idx').on(table.productId), index('product_embeddings_embedding_cosine_idx').using('hnsw', table.embedding.op('vector_cosine_ops'))]);

export const carts = pgTable('carts', { _id: uuid('id').primaryKey().defaultRandom(), userId: uuid('user_id').unique().references(() => users._id, { onDelete: 'cascade' }), guestCartId: text('guest_cart_id').unique(), ...timestamps });
export const cartItems = pgTable('cart_items', { _id: uuid('id').primaryKey().defaultRandom(), cartId: uuid('cart_id').notNull().references(() => carts._id, { onDelete: 'cascade' }), productId: uuid('product_id').notNull().references(() => products._id, { onDelete: 'cascade' }), quantity: integer('quantity').notNull().default(1) }, (table) => [unique('cart_items_cart_product_unique').on(table.cartId, table.productId)]);
export const orders = pgTable('orders', { _id: uuid('id').primaryKey().defaultRandom(), userId: uuid('user_id').notNull().references(() => users._id), orderNo: text('order_no').notNull().unique(), shippingAddress: jsonb('shipping_address').$type<ShippingAddress>().notNull(), paymentMethod: paymentMethodEnum('payment_method').notNull(), paymentStatus: paymentStatusEnum('payment_status').notNull().default('pending'), status: orderStatusEnum('status').notNull().default('placed'), statusHistory: jsonb('status_history').$type<StatusHistoryEntry[]>().notNull().default([]), subtotal: real('subtotal').notNull(), deliveryFee: real('delivery_fee').notNull().default(0), tax: real('tax').notNull(), total: real('total').notNull(), ...timestamps }, (table) => [index('orders_user_id_idx').on(table.userId)]);
export const orderItems = pgTable('order_items', { _id: uuid('id').primaryKey().defaultRandom(), orderId: uuid('order_id').notNull().references(() => orders._id, { onDelete: 'cascade' }), productId: uuid('product_id').notNull().references(() => products._id), name: text('name').notNull(), image: text('image').notNull(), originalPrice: real('original_price').notNull(), discountPercent: real('discount_percent').notNull().default(0), salePrice: real('sale_price').notNull(), quantity: integer('quantity').notNull(), isReviewed: boolean('is_reviewed').notNull().default(false) }, (table) => [index('order_items_order_id_idx').on(table.orderId)]);
export const reviews = pgTable('reviews', { _id: uuid('id').primaryKey().defaultRandom(), userId: uuid('user_id').notNull().references(() => users._id), orderId: uuid('order_id').notNull().references(() => orders._id), orderItemId: uuid('order_item_id').notNull().unique().references(() => orderItems._id), productId: uuid('product_id').notNull().references(() => products._id), rating: integer('rating').notNull(), comment: text('comment'), ...timestamps }, (table) => [index('reviews_product_id_idx').on(table.productId), index('reviews_user_id_idx').on(table.userId)]);
