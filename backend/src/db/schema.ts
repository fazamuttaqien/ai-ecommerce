...
export const productInteractionTypeEnum = pgEnum('product_interaction_type', ['view', 'homepage_click']);
...
export const products = pgTable('products', {
  _id: uuid('id').primaryKey().defaultRandom(), userId: uuid('user_id').notNull().references(() => users._id), categoryId: uuid('category_id').notNull().references(() => categories._id), name: text('name').notNull(), brand: text('brand').notNull(), slug: text('slug').notNull().unique(), description: text('description'), images: text('images').array().notNull().default([]), originalPrice: real('original_price').notNull(), salePrice: real('sale_price').notNull().default(0), discountPercent: real('discount_percent').notNull().default(0), discountLabel: text('discount_label'), unit: text('unit').notNull().default('pc'), stockCount: integer('stock_count').notNull().default(0), ratingAverage: real('rating_average').notNull().default(0), reviewCount: integer('review_count').notNull().default(0), isActive: boolean('is_active').notNull().default(true), ...timestamps,
...
export const productInteractions = pgTable('product_interactions', {
  _id: uuid('id').primaryKey().defaultRandom(), userId: uuid('user_id').notNull().references(() => users._id, { onDelete: 'cascade' }), productId: uuid('product_id').notNull().references(() => products._id, { onDelete: 'cascade' }), type: productInteractionTypeEnum('type').notNull(), createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => [index('product_interactions_user_created_idx').on(table.userId, table.createdAt), index('product_interactions_user_product_type_idx').on(table.userId, table.productId, table.type)]);
