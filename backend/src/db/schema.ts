...
export const productInteractions = pgTable(
  'product_interactions',
  {
    ...
    productId: uuid('product_id')
      .notNull()
      .references(() => products._id, { onDelete: 'cascade' }),
    type: productInteractionTypeEnum('type').notNull(),
    createdAt: timestamp('created_at').notNull().defaultNow(),
  },
  (table) => [
    index('product_interactions_user_created_idx').on(
      table.userId,
      table.createdAt,
    ),
    index('product_interactions_user_product_type_idx').on(
      table.userId,
      table.productId,
      table.type,
    ),
