...
async function request(
  ...
  const body: Record<string, unknown> | null = await res
    .json()
    .catch(() => null);
...
async function testLoginHappyPath(): Promise<void> {
  ...
  if (status !== 200)
    throw new Error(`login happy path: expected 200, got ${status}`);
  ...
  if (!(body?.user as Record<string, unknown>)?._id)
    throw new Error('login happy path: missing _id');
...
async function testLoginWrongPassword(): Promise<void> {
  ...
  if (status !== 401)
    throw new Error(`wrong password: expected 401, got ${status}`);
}
...
async function testLoginNonExistentEmail(): Promise<void> {
  ...
  if (status !== 401)
    throw new Error(`non-existent email: expected 401, got ${status}`);
}
...
async function testLoginMissingPassword(): Promise<void> {
  ...
  if (status !== 400)
    throw new Error(`missing password: expected 400, got ${status}`);
}

async function testLoginEmptyBody(): Promise<void> {
  const { status } = await request('POST', '/auth/login', { body: {} });
  if (status !== 400)
    throw new Error(`empty body: expected 400, got ${status}`);
}
